...
float2 cloSegment( in float2 p, in float2 a, in float2 b, in float th )
{
    float2 ba = b-a;
    float2 pa = p-a;
...
// distance to segment
float sdSegment( in float2 p, in float2 a, in float2 b, in float th )
{
    float2 ba = b-a;
    float2 pa = p-a;
    float h = clamp( dot(pa,ba)/dot(ba,ba), 0.0, 1.0 );
    ...
    col = float4(lerp( col, float3(0.0,0.0,0.0), 1.0-smoothstep(0.002, 0.005,abs(d)-0.005) ), 0.0);
